...
-- Tabla: TipoEspacio
CREATE TABLE TipoEspacio (
    id_tipo_espacio INT IDENTITY(1,1) PRIMARY KEY,
    nombre VARCHAR(255) NOT NULL
);

-- Tabla: EspaciosTrabajo
CREATE TABLE EspaciosTrabajo (
    id_espacio INT IDENTITY(1,1) PRIMARY KEY,
    nombre VARCHAR(255) NOT NULL,
    capacidad_maxima INT NOT NULL,
    id_tipo_espacio INT NOT NULL,
    CONSTRAINT FK_EspaciosTrabajo_TipoEspacio 
        FOREIGN KEY (id_tipo_espacio) REFERENCES TipoEspacio(id_tipo_espacio)
);
...
CREATE TABLE Reservas (
    ...
    CONSTRAINT CK_Reservas_Estado CHECK (estado IN ('Pendiente', 'Confirmada', 'Cancelada', 'Utilizada')),
    CONSTRAINT FK_Reservas_EspaciosTrabajo 
        FOREIGN KEY (id_espacio) REFERENCES EspaciosTrabajo(id_espacio),
    CONSTRAINT FK_Reservas_Clientes 
        FOREIGN KEY (id_cliente) REFERENCES Clientes(id_cliente)
);

-- Tabla: PlanesSuscripcion
CREATE TABLE PlanesSuscripcion (
    ...
    horas_mensuales INT NOT NULL,
    costo_mensual FLOAT NOT NULL,
    CONSTRAINT FK_PlanesSuscripcion_TipoEspacio 
        FOREIGN KEY (id_tipo_espacio) REFERENCES TipoEspacio(id_tipo_espacio)
);

-- Tabla: UsoHorasSuscripcion
CREATE TABLE UsoHorasSuscripcion (
    id_cliente INT NOT NULL,
    id_plan INT NOT NULL,
    horas_utilizadas INT NOT NULL,
    PRIMARY KEY (id_cliente, id_plan),
    CONSTRAINT FK_UsoHorasSuscripcion_Clientes 
        FOREIGN KEY (id_cliente) REFERENCES Clientes(id_cliente),
    CONSTRAINT FK_UsoHorasSuscripcion_PlanesSuscripcion 
        FOREIGN KEY (id_plan) REFERENCES PlanesSuscripcion(id_plan)
);
...
CREATE TABLE PagosReservas (
    ...
    id_reserva INT NOT NULL,
    PRIMARY KEY (id_pago, id_reserva),
    CONSTRAINT FK_PagosReservas_Pagos 
        FOREIGN KEY (id_pago) REFERENCES Pagos(id_pago),
    CONSTRAINT FK_PagosReservas_Reservas 
        FOREIGN KEY (id_reserva) REFERENCES Reservas(id_reserva)
);
...
CREATE TABLE PagosPlanes (
    ...
    id_plan INT NOT NULL,
    PRIMARY KEY (id_pago, id_plan),
    CONSTRAINT FK_PagosPlanes_Pagos 
        FOREIGN KEY (id_pago) REFERENCES Pagos(id_pago),
    CONSTRAINT FK_PagosPlanes_PlanesSuscripcion 
        FOREIGN KEY (id_plan) REFERENCES PlanesSuscripcion(id_plan)
);
...
CREATE TRIGGER tr_Reservas_CheckHoras
ON Reservas
AFTER INSERT, UPDATE
AS
BEGIN
    ...
    -- Verificar para cada cliente involucrado en la operación
    IF EXISTS (
        SELECT 1
        FROM (
            SELECT r.id_cliente,
                   SUM(DATEDIFF(HOUR, r.fecha_inicio, r.fecha_fin)) AS total_hours
            FROM Reservas r
            WHERE r.estado IN ('Pendiente', 'Confirmada', 'Utilizada')
              AND r.id_cliente IN (SELECT DISTINCT id_cliente FROM inserted)
            GROUP BY r.id_cliente
        ) AS t
        JOIN UsoHorasSuscripcion uhs ON t.id_cliente = uhs.id_cliente
        JOIN PlanesSuscripcion ps ON uhs.id_plan = ps.id_plan
        WHERE t.total_hours > ps.horas_mensuales
    )
    BEGIN
       RAISERROR('El cliente ha excedido la cantidad de horas disponibles en su plan de suscripcion', 16, 1);
...
CREATE TRIGGER tr_UsoHorasSuscripcion_Update
ON UsoHorasSuscripcion
AFTER UPDATE
AS
BEGIN
...
BEGIN
    ...
    SELECT
        te.nombre AS [TIPO DE ESPACIO],
        COUNT(r.id_reserva) AS [Cant. Reservas],
        ISNULL(SUM(DATEDIFF(HOUR, r.fecha_inicio, r.fecha_fin)), 0) AS [Total horas],
        COUNT(DISTINCT r.id_cliente) AS [Cant. Clientes],
        ROUND(
            (1.0 * ISNULL(SUM(DATEDIFF(HOUR, r.fecha_inicio, r.fecha_fin)), 0) / 
             (COUNT(e.id_espacio) * 24 * DATEDIFF(DAY, @fecha_desde, @fecha_hasta))
            ) * 100, 2) AS [Tasa ocupacion]
    FROM Reservas r
    JOIN EspaciosTrabajo e ON r.id_espacio = e.id_espacio
    JOIN TipoEspacio te ON e.id_tipo_espacio = te.id_tipo_espacio
    WHERE r.fecha_inicio BETWEEN @fecha_desde AND @fecha_hasta
