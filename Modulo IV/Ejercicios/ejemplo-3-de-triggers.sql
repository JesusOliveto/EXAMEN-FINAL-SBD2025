...
-- Asegurar consistencia entre columna "egresado" en matr�culas y datos de la base de datos
...
-- 2. Determinaci�n de triggers para asegurar consistencia en base a la consulta anterior
...
------------------------------------------------------------------------------------------------------
-- carreras		No						No						Propagar actualizaci�n (cambio de nota de aprobaci�n)
------------------------------------------------------------------------------------------------------
-- materias		Propagar actualizaci�n	Propagar actualizaci�n	Propagar actualizaci�n (cambio en obligatoriedad, de carrera)
------------------------------------------------------------------------------------------------------
-- examenes		Propagar actualizaci�n	Propagar actualizaci�n	Propagar actualizaci�n (cambio de nota de examen, carrera, materia, legajo)
------------------------------------------------------------------------------------------------------
...
--   select *
     from matriculas m 
          join carreras c
	        on m.cod_carrera = c.cod_carrera
    -- restringir las matr�culas a analizar 
    where m.egresado = 'N'
	  and exists (select *
	                from inserted i
					     join materias ma1
...
/*
   if @@rowcount > 1
      begin
	     raiserror ('No se puede modificar m�s de examen', 16, 1)
		 rollback
		 return
	  end
*/	  
...
begin
   if exists (select *
			   ...
			   where i.egresado = 'S')
      begin
	     raiserror('Una nueva matr�cula no puede ser insertada con el estado egresado', 16, 1)
		 rollback
		 return
	  end
